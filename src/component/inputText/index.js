...
export default function InputText({ className, errorMessage, type = 'text', ...props }) {
    ...
    const handleMouseOver = useCallback(() => setMouseOver(state => !state), [])
    return (
                ...
                <div className="absolute right-3 top-3" style={{ userSelect: 'none' }}>
                    {isPassword ? (
                        <div>
                            ...
                            {showPassword ?
                                <Eye onClick={handleShowPassword} size={24} color={ colors.slate[500]} ></Eye> :
                                <EyeSlash onClick={handleShowPassword} size={24} color={colors.slate[400]}></EyeSlash>
                            }
                            
                        </div>
                        
                    ):
                        errorMessage && <WarningCircle size={20} color={colors.red[600]}></WarningCircle>
                    }
